namespace Battleships.WebServices.Controllers
{
    ...
    public class GamesController : BaseApiController
    {
        ...
        public IHttpActionResult GetOwnGames()
        {
            var userId = this.userIdProvider.GetUserId();
            var userGames = this.Data.Games
                .All()
                .Where(g => g.State != GameState.WaitingForPlayer
                && g.State != GameState.WonByOne
                && g.State != GameState.WonByTwo
                && (userId == g.PlayerOneId || userId == g.PlayerTwoId))
                .Select(g => g.Id);

            return this.Ok(userGames);
        }
